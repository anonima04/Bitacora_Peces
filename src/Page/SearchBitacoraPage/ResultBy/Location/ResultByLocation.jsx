...
const ResultByLocation = ({ busqueda }) => {
  ...
  const EliminarBitacora = async (idBitacora, muestreos) => {
    try {
      if (muestreos && muestreos.length > 0) {
        alert("No se puede eliminar la bitácora porque tiene muestreos asociados.");
        return;
      }
      ...
      setResultados((prevResults) => prevResults.filter((item) => item.id !== idBitacora));
  ...
  const EliminarMuestreo = async (idMuestreo, especies, idBitacora) => {
    try {
      if (especies && especies.length > 0) {
        alert("No se puede eliminar el muestreo porque tiene especies asociadas.");
        return;
      }
      ...
      setResultados((prevResults) =>
        prevResults.map((item) =>
          item.id === idBitacora
            ? {
                ...item,
                muestreos: item.muestreos.filter((muestreo) => muestreo.id !== idMuestreo),
              }
            : item
        )
      );
  ...
  const EliminarEspecie = async (idMuestreo, idEspecie) => {
    try {
        ...
        prevResults.map((item) => ({
          ...
          muestreos: item.muestreos.map((muestreo) =>
            muestreo.id === idMuestreo
              ? {
                  ...muestreo,
                  especies: muestreo.especies.filter((especie) => especie.id !== idEspecie),
                }
              : muestreo
          ),
        }))
      ...
      alert(`Especie con ID ${idEspecie} eliminada del muestreo ${idMuestreo}.`);
    } catch (error) {
        ...
        bitacoras.map((bitacora) => (
          <div key={bitacora.id} className="bitacora-card">
            ...
            <button
              ...
              onClick={() =>
                EliminarMuestreo(bitacora.muestreo.id, bitacora.muestreo.especies, bitacora.id)
              }
            >
              Eliminar Muestreo
            </button>
            <p>
              <strong>Condición Climática:</strong> {bitacora.muestreo.CONDICION_CLIMATICA}
            </p>
            <p>
              <strong>Descripción del Hábitat:</strong> {bitacora.muestreo.DESCRIPCION_HABITAD}
            </p>
            <p>
              <strong>Observaciones:</strong> {bitacora.muestreo.OBSERVACION}
            </p>
            <p>
              <strong>Fecha creación:</strong> {bitacora.muestreo.FECHA_CREACION}
            </p>

            <div className="imagenes-container">
              {bitacora.muestreo.FOTOGRAFIAS && bitacora.muestreo.FOTOGRAFIAS.length > 0 ? (
                bitacora.muestreo.FOTOGRAFIAS.map((foto, idx) => (
                  <img key={idx} src={foto} alt={`Foto ${idx}`} className="foto" />
                ))
              ) : (
            ...
            <h5>Especies</h5>
            {bitacora.muestreo.especies && bitacora.muestreo.especies.length > 0 ? (
              bitacora.muestreo.especies.map((especie, idx) => (
                <div key={idx} className="especie">
                  ...
                  <p>
                    <strong>Nombre común: </strong>{especie.NOMBRE_COMUN}
                  </p>
                  <p>
                  ...
                  <div className="imagenes-container">
                    {especie.FOTOGRAFIAS && especie.FOTOGRAFIAS.length > 0 ? (
                      especie.FOTOGRAFIAS.map((foto, idx) => (
                        <img key={idx} src={foto} alt={`Especie Foto ${idx}`} className="foto" />
                      ))
                    ) : (
                      <p>No hay fotos disponibles</p>
                    )}
