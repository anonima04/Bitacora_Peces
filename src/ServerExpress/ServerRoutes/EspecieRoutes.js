...
router.get("/bitacoras/:idBitacora", async (req, res) => {
  ...
  try {
    ...
    if (snapshot.empty) {
      return res
        .status(404)
        .json({ message: "No se encontraron ESPECIES en esta bitácora" });
    }

    // Mapea los documentos obtenidos
    const bitacorasRecursos = snapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
  ...
  try {
    ...
    if (snapshot.empty) {
      return res.status(404).json({
        message: "No se encontraron ESPECIES guardas por esta persona",
      });
    }
...
router.get("/muestra/:idMuestra", async (req, res) => {
  ...
  try {
    ...
    if (snapshot.empty) {
      return res
        .status(404)
        .json({ message: "No se encontraron ESPECIES guardas en muestra" });
    }
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!docSnapshot.exists) {
      return res
        .status(404)
        .json({ message: "No se encontró la especie con ese ID" });
    }
...
router.delete("/eliminar/:idEspecie", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Especie con ID ${idEspecie} eliminada exitosamente` });
  } catch (error) {
